...
export default function DoctorDashboard() {
  ...
  const loadData = async () => {
    try {
      const appts = await appointmentAPI.getMyAppointments(true);
      setAppointments(appts.slice(0, 10));

      const allAppts = await appointmentAPI.getMyAppointments(false);
  ...
  return (
    <DashboardLayout navItems={navItems}>
      <div>
        <h1 className="text-3xl font-bold text-gray-900 mb-6">Welcome, Dr. {user.name}!</h1>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <Link href="/doctor/appointments" className="text-sm text-blue-600 hover:text-blue-700 font-medium">
              View All
            </Link>
          </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <p className="text-gray-900 font-medium">
                        {format(new Date(apt.date), 'EEEE, MMM dd, yyyy')} at {apt.startTime}
                      </p>
                      {apt.reason && (
                    ...
                    <div className="flex flex-col space-y-2 ml-4">
                      ...
                      {apt.status === 'confirmed' && (
                        <Link
                          ...
                          className="text-sm bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded font-medium text-center"
                        >
                          View Details
                        </Link>
                      )}
